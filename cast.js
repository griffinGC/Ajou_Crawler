...
request(url, function(err, response, body){
	...
	where = $('h3.api_title').text();
    // console.log(where);
    todayTemp = $('span.todaytemp').first().text();
    // console.log(todayTemp);
    txt = $('p.cast_txt').first().text();
    // console.log(txt);
    totalText = `오늘 아주대 ${where}는 ${txt}.`+'\n'+`현재기온은 ${todayTemp}도 입니다.`;
    ...
    request(url2, function(err, response, body){
        ...
        dust = dust + '㎍/㎥';
        result = `오늘 미세먼지 수치는 ${dust}이고 상태는 ${result}입니다.`
        //미세먼지
        console.log(result);
        });
});
